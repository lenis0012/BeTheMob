...
public class HerdEntity {
	...
	public HerdEntity(int EntityID, Location loc, String name, int itemInHand,
			EntityType type, Player leader) {
	...
	public HerdEntity(int EntityID, Location loc, int itemInHand,
			EntityType type, Player leader) {
		this.setEntityID(EntityID);
		this.setLocation(loc);
		this.setItemInHand(itemInHand);
	...
	public void spawn(World world) {
		if (isPlayer) {
			...
			NetworkUtil
					.sendGlobalPacket(gen.getNamedEntitySpawnPacket(), world);
		} else {
	...
	/**
	 * Kill the entity with the normal fall over animation (Warning kills and
	 * despawns entity)
	 */
	public void kill() {
		NetworkUtil.sendGlobalPacket(gen.getEntityStatusPacket((byte) 3),
				loc.getWorld());
	}
	...
	/**
	 * @param item
	 *            Item to change the currently held item to slot 0 is the item
	 *            in hand
	 */
	public void changeItem(int new_slot, org.bukkit.inventory.ItemStack item) {
		...
		NetworkUtil.sendGlobalPacket(
				gen.getEntityEquipmentPacket(0, bukkitstack), loc.getWorld());
	}

	/**
	 * @param armor
	 *            What type of armor to put on entity
	 * @param slot
	 *            Which armour slot 1 boots 2 leggings 3 chestplate 4 helmet
	 */
	public void changeArmor(int slot, org.bukkit.inventory.ItemStack armor) {
		...
		NetworkUtil
				.sendGlobalPacket(
						gen.getEntityEquipmentPacket(slot, bukkitstack),
						loc.getWorld());
	}
	 ...
	 * 
	 * @param from
	 *            - The location moving from
	 * @param to
	 *            - the location to move to
	 * @param moved
	 *            - whether the entity moved, or simply changed pitch/yaw
	 */
	public void move(Location to, boolean moved) {
		...
		if (moved) {
			...
			NetworkUtil.sendGlobalPacket(gen.getEntityMoveLookPacket(movement),
					to.getWorld());
		...
		NetworkUtil.sendGlobalPacket(gen.getEntityHeadRotatePacket(),
				to.getWorld());
	...
	public void teleport(Location loc) {
		this.loc = loc;
		NetworkUtil.sendGlobalPacket(gen.getEntityTeleportPacket(),
				loc.getWorld());
		this.movement = new Movement(loc);
	}
	...
	public void swingArm() {
		NetworkUtil.sendGlobalPacket(gen.getArmAnimationPacket(1),
				loc.getWorld());
	}
	...
	public void damageBlock(Block block) {
		NetworkUtil.sendGlobalPacket(gen.getBlockBreakAnimationPacket(block),
				loc.getWorld());
	}
	 ...
	 */
	public void damage() {
		NetworkUtil.sendGlobalPacket(gen.getArmAnimationPacket(2),
				loc.getWorld());
	}
	...
	public void knockback() {
		...
		/*
		 * short velx = 0; short vely = 5; short velz = 5;
		 * loc.setY(loc.getY()+5); loc.setZ(loc.getZ()+5);
		 * NetworkUtil.sendGlobalPacket(gen.getEntityVelocityPacket(new
		 * Vector(velx, vely, velz)), getLocation().getWorld());
		 */
	}
	...
	public void updateMetaData() {
		NetworkUtil.sendGlobalPacket(gen.getEntityMetadataPacket(),
				loc.getWorld());
	}
	...
	/**
	 * Gets the correct damaged sound If the entity is a zombie it will be a
	 * zombie one Etc.
	 */
	public Sound getHurtSound() {
		...
		if (type == EntityType.BAT) {
			...
			return random.nextBoolean() ? Sound.GHAST_SCREAM
					: Sound.GHAST_SCREAM2;
	...
	public void update(double delayBetweenUpdates) {
		if (System.currentTimeMillis() - timeLastUpdated >= delayBetweenUpdates) {
			...
			if (entityToAttack != null && !entityToAttack.isDead()) {
				if (PathfindingUtil.distanceBetweenLocs(getLocation(),
						entityToAttack.getLocation()) < 3) {
					Random rand = new Random();
					int attack = rand.nextInt(3);
					specialAttack(entityToAttack, rand.nextInt(2));
				}
			}
		}
	}

	public void specialAttack(LivingEntity target, int attacktype) {
		if (getType() == EntityType.PIG || getType() == EntityType.COW
				|| getType() == EntityType.SHEEP) {
			switch (attacktype) {
			case 0:
				target.setVelocity(target.getVelocity().add(
						new Vector(0.5, 0.8, 0.5)));
			case 1:
				Location blockbelow = new Location(getWorld(), getLocation().getBlockX(), getLocation().getBlockY()-1, getLocation().getBlockZ());
				if(blockbelow.getBlock().getType().isSolid()) {
					FallingBlock block = getWorld().spawnFallingBlock(getLocation(), blockbelow.getBlock().getType(), blockbelow.getBlock().getData());
					block.setVelocity(getLocation().getDirection().multiply(4));
				}
			}
		}
	}
