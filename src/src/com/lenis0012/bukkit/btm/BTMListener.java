...
public class BTMListener implements Listener {
	...
	public void onHerdEntityInteractedWith(HerdEntityInteractEvent evt) {
		...
		if (evt.getAction() == 1) {
			...
			evt.getEntityInteractedWith().health -= HerdUtil.getDamageDealt(evt
					.getPlayer().getItemInHand());
			if (evt.getEntityInteractedWith().health <= 0) {
				evt.getEntityInteractedWith().kill();
			}
		} else {
			if (evt.getEntityInteractedWith().getLeader().getName() == evt
					.getPlayer().getName()) {
				BeTheMob.instance.selectedentities.put(evt.getPlayer()
						.getName(), evt.getEntityInteractedWith());
			}
			/*if (evt.getEntityInteractedWith().getType() == EntityType.COW
					&& evt.getPlayer().getItemInHand().getType() == Material.BUCKET) {
				evt.getPlayer().getItemInHand().setType(Material.MILK_BUCKET);
			}
			if (evt.getEntityInteractedWith().getType() == EntityType.MAGMA_CUBE
					&& evt.getPlayer().getItemInHand().getType() == Material.BUCKET) {
				...
				public void run() {
					if (plugin.disguises.containsKey(name)) {
						...
						Bukkit.getScheduler().runTaskLaterAsynchronously(
								BeTheMob.instance, new Runnable() {
		...
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			...
			public void run() {
				if (plugin.disguises.containsKey(name)) {
					...
					if (dis.isPlayer()) { // Only switch armour for player
											// disguise types
	...
	public void onPlayerDeath(final PlayerDeathEvent event) {
		...
		if (BeTheMob.instance.getConfig().getBoolean("drop_real_item")
				&& plugin.disguises.containsKey(name)) {
			event.getDrops().clear();
			Set<ItemStack> items = BeTheMob.instance.getDropFactory().getDrops(
					BeTheMob.getApi().getDisguise(player));
			BeTheMob.instance.getLogger().info(
					"Dropping " + items.size() + " items for " + name + ".");
			if (items != null) {
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			...
			public void run() {
				...
				Bukkit.getScheduler().runTaskLater(BeTheMob.instance,
						new Runnable() {

							@Override
							public void run() {
								for (String s : BeTheMob.instance.disguises
										.keySet()) {
									Disguise dis = BeTheMob.instance.disguises
											.get(s);
									dis.spawn(player);
								}
							}
				...
				if (herd.getLeader().getName() == name) {
					for (HerdEntity hentity : herd.getHerdMembers()) {
						if (entity instanceof LivingEntity) {
							...
							hentity.setEntityToAttack(((Projectile) entity)
									.getShooter());
						}
					}
				}
