...
public class Herd {
	...
	public void populateHerd(int membercount, int radius) {
		for(int i=0;i<membercount;i++) {
			...
			Location entityloc = new Location(leader.getWorld(), leader.getLocation().getX()+rand.nextInt(radius), leader.getLocation().getY(), leader.getLocation().getZ()+rand.nextInt(radius));
			addHerdMember(new HerdEntity(BeTheMob.instance.nextID--, entityloc, 0, BeTheMob.instance.disguises.get(leader.getName()).getDisguiseType()));
		}
	}
	/**
